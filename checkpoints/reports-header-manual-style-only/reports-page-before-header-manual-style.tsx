...
function Header({
  ...
  const status = String(campaignRecord.status || "Not loaded");
  const updatedAt = String(campaignRecord.updatedAt || campaignRecord.createdAt || "");
  const readiness = stats?.verdict || "Ready";

  return (
    <section className="relative overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.055] px-6 py-10 text-center shadow-[0_24px_80px_rgba(0,0,0,0.35)] backdrop-blur-2xl">
      <div className="pointer-events-none absolute -left-24 -top-24 h-56 w-56 rounded-full bg-red-500/18 blur-3xl" />
      <div className="pointer-events-none absolute -right-24 top-8 h-56 w-56 rounded-full bg-cyan-500/10 blur-3xl" />

      <div className="relative mx-auto max-w-5xl">
        <p className="text-[11px] font-black uppercase tracking-[0.5em] text-cyan-300">
          HexaGuard Reports
        </p>

        <h1 className="mt-4 text-4xl font-black uppercase leading-[0.95] tracking-[-0.05em] text-white md:text-6xl">
          AI Risk Analysis
          <span className="mt-1 block text-red-500">Reports</span>
        </h1>

        <p className="mx-auto mt-4 max-w-3xl text-base leading-7 text-gray-300">
          Review campaign evidence, residual risks, model performance, defensive recommendations,
          and export-ready analytical reports.
        </p>

        <div className="mt-5 flex flex-wrap justify-center gap-2">
          <span className="rounded-full border border-white/10 bg-black/30 px-4 py-2 font-mono text-[11px] text-gray-300">
            Campaign: <span className="text-cyan-200">{campaignId || "Not selected"}</span>
          </span>
          ...
          <span className="rounded-full border border-white/10 bg-black/30 px-4 py-2 font-mono text-[11px] text-gray-300">
            Readiness: <span className="text-emerald-300">{readiness}</span>
          </span>

          <span className="rounded-full border border-white/10 bg-black/30 px-4 py-2 font-mono text-[11px] text-gray-300">
            Manual Evidence: <span className="text-cyan-200">{manualEvidenceCount}</span>
          </span>

          {updatedAt ? (
            <span className="rounded-full border border-white/10 bg-black/30 px-4 py-2 font-mono text-[11px] text-gray-300">
              Updated: <span className="text-gray-200">{formatDate(updatedAt)}</span>
            </span>
          ) : null}
        </div>

        <div className="mx-auto mt-6 max-w-3xl rounded-[24px] border border-white/10 bg-black/25 p-4 shadow-[0_18px_55px_rgba(0,0,0,0.28)]">
          <div className="grid gap-3 lg:grid-cols-[1fr_auto]">
            <input
              value={campaignId}
              onChange={(event) => setCampaignId?.(event.target.value)}
              placeholder="HXG-CMP-..."
              className="h-12 rounded-2xl border border-red-400/20 bg-black/30 px-5 font-mono text-sm text-white outline-none transition placeholder:text-gray-500 focus:border-red-400/50"
            />

            <button
              onClick={onLoad}
              disabled={loading}
              className="h-12 rounded-2xl bg-red-500 px-7 text-sm font-black text-white shadow-[0_0_24px_rgba(239,68,68,0.28)] transition hover:bg-red-400 disabled:cursor-not-allowed disabled:opacity-60"
            >
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-5">
            ...
            <HeaderButton onClick={onJson} icon={<FileJson size={14} />}>JSON Report</HeaderButton>
            <HeaderButton onClick={onCsv} icon={<Download size={14} />}>Spreadsheet Report</HeaderButton>
            <HeaderButton onClick={onPdf} icon={<FileText size={14} />}>PDF Report</HeaderButton>
          </div>
        </div>
      </div>
    </section>
