...
function ResultModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      <section className="relative flex max-h-[90vh] w-full max-w-6xl flex-col overflow-hidden rounded-[28px] border border-white/10 bg-[#0a0c10] shadow-[0_0_80px_rgba(0,0,0,0.7)]">
        <header className="flex items-start justify-between gap-4 border-b border-white/10 bg-white/[0.03] p-6">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-3">
              <h3 className="text-2xl font-bold text-white">Evidence Inspection</h3>
              <span className="rounded-xl border border-white/10 bg-black/35 px-3 py-1 font-mono text-[10px] uppercase tracking-widest text-gray-300">
                {result.resultId}
              </span>
              <RiskMovementBadge inputLevel={result.inputRiskLevel} outputLevel={result.outputRiskLevel} />
            </div>

            <p className="mt-3 flex flex-wrap gap-4 font-mono text-xs text-gray-500">
              <span>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-6">
          <div className="grid gap-4 md:grid-cols-4">
            <InfoLine label="Decision" value={result.decision} />
            <InfoLine label="Attack Success" value={result.attackSuccess ? "Yes" : "No"} />
            <InfoLine label="Input Risk" value={`${result.inputRiskLevel} / ${result.inputRiskScore}`} />
            <InfoLine label="Output Risk" value={`${result.outputRiskLevel} / ${result.outputRiskScore}`} />
            <InfoLine label="AI Judge" value={result.aiJudgeStatus} />
            <InfoLine label="Confidence" value={result.confidence} />
            <InfoLine label="Input Type" value={result.inputType} />
            <InfoLine label="Created At" value={formatDate(result.createdAt)} />
          </div>

          <div className="grid gap-6 xl:grid-cols-2">
            <DetailBox title="Original Prompt" value={result.prompt} />
            <DetailBox title="Mutated Prompt" value={result.mutatedPrompt} />
          </div>

          <DetailBox title="Model Response" value={result.modelResponse} />
          <DetailBox title="AI Judge Reasoning" value={result.reasoning} />

          {result.executionError ? (
            <DetailBox title="Execution Error" value={result.executionError} danger />
          ) : null}

          <DetailBox title="Backend Metadata" value={JSON.stringify(result.raw, null, 2)} />
        </div>

        <footer className="flex justify-end gap-3 border-t border-white/10 bg-white/[0.03] p-6">
          <button
            onClick={onClose}
          ...
          <button
            onClick={onExport}
            className="rounded-2xl bg-cyan-500 px-6 py-2.5 text-sm font-black text-white shadow-[0_0_22px_rgba(34,211,238,0.22)] transition hover:bg-cyan-400"
          >
            Export Evidence
          </button>
        </footer>
...
function ReportsSkeleton() {
  return (
    <main className="min-h-screen bg-[#080c0f] px-6 pt-12">
      <section className="mx-auto max-w-[1600px] space-y-6">
        <div className="h-44 animate-pulse rounded-[28px] border border-white/10 bg-white/[0.055]" />

        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 2xl:grid-cols-7">
          {Array.from({ length: 7 }).map((_, index) => (
            <div
              key={index}
              className="h-32 animate-pulse rounded-[26px] border border-white/10 bg-white/[0.055]"
            />
          ))}
        </div>
...
function MetricCard({
  ...
  return (
    <div className="rounded-[28px] border border-white/10 bg-white/[0.055] p-6 shadow-[0_25px_80px_rgba(0,0,0,0.28)] backdrop-blur-2xl">
      <div className="mb-4 flex items-start justify-between gap-4">
        <p className="text-[10px] font-black uppercase tracking-[0.18em] text-gray-500">{label}</p>
        {icon}
      </div>

      <p className={["truncate text-3xl font-light", danger ? "text-red-300" : "text-white"].join(" ")}>
        {value}
      </p>

      <p className="mt-2 truncate text-xs text-gray-500">{helper}</p>
    </div>
  );
}
...
function InfoLine({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-black/25 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
      ...
      <p className="mt-2 break-words text-sm font-semibold text-gray-100">{value || "Not available"}</p>
    </div>
  );
}
  ...
  return (
    <div
      className={[
        ...
        highlight ? "border-cyan-400/20 bg-cyan-500/10" : "border-white/10 bg-black/25",
      ].join(" ")}
    >
      <p className={["text-[10px] font-black uppercase tracking-[0.16em]", highlight ? "text-cyan-300" : "text-gray-500"].join(" ")}>
        {label}
      </p>
      <p className={["mt-2 text-sm leading-6", highlight ? "text-cyan-50/90" : "text-gray-300"].join(" ")}>
        {value || "Not available"}
      </p>
    </div>
  );
