...
const PROVIDERS = [
  {
    value: "mock",
    label: "Mock",
    helper: "Built-in demo provider. No API key required.",
  },
  {
    value: "openai",
    label: "OpenAI",
    helper: "Hosted provider. API key required.",
  },
  {
    value: "groq",
    label: "Groq",
  ...
  },
  {
    value: "ollama",
    label: "Ollama",
    helper: "Local provider. Base URL required.",
  },
  {
    value: "lmstudio",
    label: "LM Studio",
    helper: "Local OpenAI-compatible provider.",
  },
  {
    value: "custom",
    label: "Custom OpenAI-Compatible",
    helper: "Custom base URL and optional API key.",
  },
];
...
function statusTone(status: ModelStatus) {
  ...
  if (status === "needs_api_key" || status === "needs_test") return "warning";
...
function initialStatusFromForm(form: ModelFormState): ModelStatus {
  ...
  if (form.provider === "openai" || form.provider === "groq") {
    return form.apiKey || form.maskedKey ? "needs_test" : "needs_api_key";
  }

  if (form.provider === "ollama" || form.provider === "lmstudio" || form.provider === "custom") {
    return form.baseUrl ? "needs_test" : "unavailable";
  }
...
function friendlyConnectionMessage(message: string, success: boolean) {
  ...
  if (lower.includes("401") || lower.includes("auth") || lower.includes("key")) {
    return {
      ...
      suggestedFix: "Update or replace the provider API key, then test the connection again.",
    };
  }
  ...
  if (lower.includes("timeout")) {
    return {
      ...
      suggestedFix: "Check provider availability, network connectivity, or increase timeout settings.",
    };
  }
  ...
  if (lower.includes("refused") || lower.includes("unreachable") || lower.includes("failed to fetch")) {
    return {
      ...
      suggestedFix: "For local providers, make sure Ollama or LM Studio is running. For hosted providers, check the backend connection.",
    };
  }
  ...
  return {
    ...
    suggestedFix: "Review provider credentials and configuration, then test again.",
  };
}
...
function mapBackendStatus(value: string, enabled = true): ModelStatus {
  ...
  if (status === "needs_api_key" || status === "not_configured") return "needs_api_key";
  if (status === "needs_test" || status === "untested") return "needs_test";
  if (status === "connection_error" || status === "error") return "connection_error";
  if (status === "unavailable") return "unavailable";
  if (status === "testing") return "testing";

  return enabled ? "needs_test" : "disabled";
}
...
function mapCampaignModel(item: CampaignModelOption, index: number): RegistryModel {
  ...
  const ready = item.available_for_campaigns && item.configured;
  ...
  return {
    ...
    status: provider === "mock" ? "demo_only" : ready ? "ready" : item.configured ? "connection_error" : "needs_api_key",
    enabled: item.available_for_campaigns,
    usageScope: "both",
    isDefault: index === 0,
    lastTestedAt: "",
    latencyMs: undefined,
    lastError: ready || provider === "mock" ? "" : "Provider configuration is incomplete.",
...
function SummaryCard({
  ...
  return (
    <article className={`rounded-[20px] border p-5 shadow-[0_18px_45px_rgba(0,0,0,0.18)] ${className}`}>
    ...
    <article className="rounded-[22px] border border-white/[0.06] bg-[#27292a] p-5 shadow-[0_18px_45px_rgba(0,0,0,0.18)]">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 text-sm leading-6 text-[#a9a9a9]">
            {provider.helper || "Provider connection for target models."}
          </p>
        </div>
      ...
      <div className="mt-5 grid gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-[16px] border border-white/[0.06] bg-[#1f2122] p-4">
          ...
          <p className="mt-2 text-2xl font-black text-emerald-300">{readyCount}</p>
        </div>
      </div>
      ...
      <button
        type="button"
        onClick={onConfigure}
        className="mt-5 rounded-full border border-[#4ad7ff]/30 px-4 py-2 text-xs font-black text-[#4ad7ff] transition hover:bg-[#4ad7ff]/10"
      >
        Configure
      </button>
    </article>
      ...
      <section className="max-h-[90vh] w-full max-w-3xl overflow-auto rounded-[24px] border border-white/[0.08] bg-[#202323] shadow-[0_30px_100px_rgba(0,0,0,0.55)]">
        <div className="border-b border-white/[0.08] bg-[#27292a] px-6 py-5">
          ...
          <p className="mt-2 text-sm leading-6 text-[#a9a9a9]">
            ...
            Full API keys are never shown after saving.
          </p>
        </div>
        ...
        <div className="grid gap-4 p-6 md:grid-cols-2">
          ...
          <label>
            ...
            <p className="mt-2 text-xs text-[#727272]">
              Stored display: {form.maskedKey || "No key stored in frontend"}
            </p>
          </label>
    ...
    const backendProviders =
      providers.length > 0
        ? providers.map((provider) => ({
            ...
            helper: provider.description || "Backend provider configuration.",
    ...
    try {
      ...
      const nextStatus: ModelStatus = success
        ? model.provider === "mock"
          ? "demo_only"
          : "ready"
        : friendly.message.includes("Authentication")
          ? "needs_api_key"
    ...
    <main className="hxg-background min-h-screen px-4 py-6 text-white sm:px-6 xl:px-8">
      <div className="mx-auto w-full max-w-[1500px] space-y-7">
            ...
            <div>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                {["1. Providers", "2. Models", "3. Connection Test", "4. Ready for Testing"].map((chip) => (
                  <span key={chip} className="rounded-full border border-white/[0.08] bg-[#27292a] px-4 py-2 text-xs font-black text-[#d4d4d4]">
                    {chip}
                  </span>
                ))}
              </div>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          <SummaryCard label="Configured Models" value={summary.total} helper="All registered target models" />
          <SummaryCard label="Ready for Testing" value={summary.ready} helper="Enabled and selectable models" tone="success" />
          <SummaryCard label="Needs Attention" value={summary.attention} helper="Broken, disabled, or not configured" tone="warning" />
          <SummaryCard label="Default Target" value={summary.defaultModel} helper="Used by Manual and Campaigns by default" tone="info" />
        </section>
        ...
        <section className="rounded-[23px] border border-white/[0.04] bg-[#27292a]/95 p-6">
          <div className="mb-5">
            ...
            <h2 className="mt-1 text-2xl font-black text-white">Provider Status</h2>
          </div>
        ...
        <section className="rounded-[23px] border border-white/[0.04] bg-[#27292a]/95 p-6">
          <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-black text-white">Target Models</h2>
              <p className="mt-2 text-sm leading-6 text-[#a9a9a9]">
                Configure and validate models used by Manual Red Teaming and Campaigns. Analysis belongs in Reports.
              </p>
            </div>
            ...
            <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
              ...
              >
                ...
                {["ready", "needs_api_key", "needs_test", "connection_error", "unavailable", "disabled", "demo_only", "testing"].map((status) => (
                    ...
                    <tr key={model.id} className="border-t border-white/[0.06] bg-[#27292a]/60 transition hover:bg-white/[0.035]">
                      <td className="px-4 py-4">
                        <button
                          ...
                          className="text-left"
                        >
                          <p className="font-black text-white hover:text-[#4ad7ff]">
                            {model.displayName}
                          </p>
                          <p className="mt-1 text-xs text-[#727272]">
                            {isReadyForTesting(model) ? "Available for testing" : "Unavailable until configured"}
                          </p>
                        </button>
                      </td>
                      ...
                      <td className="px-4 py-4 font-mono text-xs text-[#4ad7ff]">
                        {model.modelId}
                      </td>
                      ...
                      <td className="px-4 py-4 font-mono text-xs text-[#d4d4d4]">
                      ...
                      <td className="relative px-4 py-4">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            type="button"
                            onClick={() => setDetailsModel(model)}
                            className="h-10 rounded-[12px] border border-white/[0.08] bg-[#1f2122] px-4 text-xs font-black text-white transition hover:border-[#4ad7ff]/30"
                          >
                            View
                          </button>
                          ...
                          <div className="relative">
                            ...
                            {openMenuId === model.id ? (
                              <div className="absolute right-0 top-12 z-20 w-44 overflow-hidden rounded-[14px] border border-white/[0.08] bg-[#1f2122] shadow-[0_18px_50px_rgba(0,0,0,0.45)]">
                                <button
                                  type="button"
                                  onClick={() => {
                                    openEditModal(model);
          ...
          <div className="mt-5 grid gap-3">
            {connectionChecks.length === 0 ? (
              ...
              connectionChecks.map((check) => (
                <div key={check.id} className="grid gap-3 rounded-[18px] border border-white/[0.06] bg-[#1f2122] p-4 md:grid-cols-[1fr_auto]">
                  <div>
