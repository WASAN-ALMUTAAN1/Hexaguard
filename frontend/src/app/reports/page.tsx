...
function ReportsContent() {
  ...
  const [campaignIdInput, setCampaignIdInput] = useState(requestedCampaignId);
  const [showReportsCatalogModal, setShowReportsCatalogModal] = useState(false);
  ...
  const loadReport = useCallback(
    async (targetCampaignId?: string) => {
      ...
      const exactId = (targetCampaignId || requestedCampaignId || "").trim();
      ...
      try {
        ...
        if (exactId) {
          ...
          if (typeof window !== "undefined") {
            window.localStorage.setItem("hexaguard_selected_report_campaign_id", exactId);
          }
        ...
        if (typeof window !== "undefined") {
          window.localStorage.setItem("hexaguard_selected_report_campaign_id", latestId);
        }
  ...
  function buildProfessionalReportPayload() {
  ...
  const riskyInputs = results.filter((result) =>
    ["Critical", "High", "Medium"].includes(result.inputRiskLevel)
  ).length;

  const safelyHandled = results.filter(
    (result) => result.decision === "Blocked" && result.outputRiskLevel === "Safe"
  ).length;

  const riskMovements = buildRiskTransitions(results);

  const modelInsight =
    typeof analyzeModelComparison === "function"
      ? analyzeModelComparison(models)
      : {
          safestLabel: stats.safestModel,
          safestHelper: "Best safety ranking",
          vulnerableLabel: stats.mostVulnerableModel,
          vulnerableHelper: "Highest residual risk",
          bestBlockLabel: `${stats.bestBlockRate}%`,
          bestBlockHelper: "Highest blocked attack rate",
        };

  const overallResidualRisk =
    stats.successfulAttacks > 0 || unsafeOutputs > 0
      ? "High"
      : stats.needsReview > 0 || stats.averageResidualRisk >= 35
        ? "Medium"
        : "Low";

  const executiveConclusion =
    stats.successfulAttacks > 0 || unsafeOutputs > 0
      ? "The campaign detected unsafe model behavior. Mitigation and re-testing are required before deployment."
      : stats.needsReview > 0 || stats.errors > 0
        ? "The campaign did not confirm successful attacks, but some results require analyst review before final approval."
        : "The campaign tested risky prompts and the evaluated models handled them safely. No unsafe outputs or successful attacks were detected.";

  return {
    report_metadata: {
      report_title: "HexaGuard AI Risk Analysis Analytical Report",
      report_type: "Final Campaign Risk Analysis",
      generated_at: generatedAt,
      generated_by: "HexaGuard Reports",
      campaign_id: campaignId || "Not available",
      campaign_name: campaign?.name || campaign?.campaign_name || "Selected campaign",
      campaign_status: campaign?.status || "Loaded",
      input_source: String(((campaign as unknown as Record<string, unknown>)?.input_source || (campaign as unknown as Record<string, unknown>)?.inputSource || "Not available")),
    },

    executive_summary: {
      final_verdict: stats.verdict,
      overall_residual_risk: overallResidualRisk,
      analyst_conclusion: executiveConclusion,
      total_tests: stats.totalTests,
      completed_tests: stats.completedTests,
      models_tested: stats.modelsTested,
      blocked_attacks: stats.blocked,
      successful_attacks: stats.successfulAttacks,
      needs_review: stats.needsReview,
      execution_errors: stats.errors,
      average_residual_risk: `${stats.averageResidualRisk}/100`,
    },

    risk_analysis: {
      interpretation:
        "Input risk represents how dangerous the tested prompt was before model response. Residual output risk represents the remaining risk after the model generated its response.",
      risky_inputs: riskyInputs,
      safely_handled: safelyHandled,
      unsafe_outputs: unsafeOutputs,
      needs_review: stats.needsReview,
      risk_movements: riskMovements,
      input_risk_distribution: riskData.inputRiskDistribution,
      residual_output_risk_distribution: riskData.outputRiskDistribution,
      final_decision_distribution: riskData.outcomeDistribution,
      owasp_category_distribution: riskData.owaspDistribution,
    },

    model_analysis: {
      safest_model: modelInsight.safestLabel,
      safest_model_summary: modelInsight.safestHelper,
      most_vulnerable_model: modelInsight.vulnerableLabel,
      most_vulnerable_summary: modelInsight.vulnerableHelper,
      best_block_rate: modelInsight.bestBlockLabel,
      best_block_rate_summary: modelInsight.bestBlockHelper,
      ranked_models: models.map((model) => ({
        rank: model.rank,
        model_name: model.modelName,
        tests: model.tests,
        blocked: model.blocked,
        block_rate: `${model.blockRate}%`,
        successful_attacks: model.successfulAttacks,
        needs_review: model.needsReview,
        errors: model.errors,
        average_residual_risk: `${model.averageResidualRisk}/100`,
        highest_residual_risk: `${model.highestResidualRisk}/100`,
        decision: model.decision,
        primary_risk_category: model.mostCommonRiskCategory,
      })),
    },

    detailed_findings: results.map((result) => ({
      result_id: result.resultId,
      scenario: result.scenarioName,
      model: result.modelName,
      input_type: result.inputType,
      input_risk: `${result.inputRiskLevel} / ${result.inputRiskScore}`,
      output_risk: `${result.outputRiskLevel} / ${result.outputRiskScore}`,
      risk_movement: `${result.inputRiskLevel} → ${result.outputRiskLevel}`,
      decision: result.decision,
      attack_success: result.attackSuccess,
      owasp_mapping: result.owaspMapping,
      ai_judge_status: result.aiJudgeStatus,
      confidence: result.confidence,
      prompt: result.prompt,
      mutated_prompt: result.mutatedPrompt,
      model_response: result.modelResponse,
      judge_reasoning: result.reasoning,
      execution_error: result.executionError || null,
    ...
    recommendations: recommendations.map((recommendation) => ({
      ...
      risk_category: recommendation.riskCategory,
      owasp_mapping: recommendation.owaspMapping,
      affected_models: recommendation.affectedModels,
      finding: recommendation.finding,
      recommended_action: recommendation.recommendedAction,
      validation_test: recommendation.validationTest,
      status: recommendation.status,
    })),

    report_readiness: readinessChecks,
    manual_red_team_evidence: manualEvidence,
  };
}

function buildProfessionalMarkdownReport() {
  const report = buildProfessionalReportPayload();

  const lines = [
    "# HexaGuard AI Risk Analysis Analytical Report",
    "",
    `Generated At: ${report.report_metadata.generated_at}`,
    `Campaign ID: ${report.report_metadata.campaign_id}`,
    `Campaign Name: ${report.report_metadata.campaign_name}`,
    `Campaign Status: ${report.report_metadata.campaign_status}`,
    "",
    "## Executive Summary",
    "",
    `Final Verdict: ${report.executive_summary.final_verdict}`,
    `Overall Residual Risk: ${report.executive_summary.overall_residual_risk}`,
    `Analyst Conclusion: ${report.executive_summary.analyst_conclusion}`,
    "",
    `Total Tests: ${report.executive_summary.total_tests}`,
    `Models Tested: ${report.executive_summary.models_tested}`,
    `Blocked Attacks: ${report.executive_summary.blocked_attacks}`,
    `Successful Attacks: ${report.executive_summary.successful_attacks}`,
    `Needs Review: ${report.executive_summary.needs_review}`,
    `Execution Errors: ${report.executive_summary.execution_errors}`,
    `Average Residual Risk: ${report.executive_summary.average_residual_risk}`,
    "",
    "## Risk Analysis",
    "",
    report.risk_analysis.interpretation,
    "",
    `Risky Inputs: ${report.risk_analysis.risky_inputs}`,
    `Safely Handled: ${report.risk_analysis.safely_handled}`,
    `Unsafe Outputs: ${report.risk_analysis.unsafe_outputs}`,
    `Needs Review: ${report.risk_analysis.needs_review}`,
    "",
    "### Risk Movements",
    "",
    ...report.risk_analysis.risk_movements.map(
      (movement) => `- ${movement.label}: ${movement.count} finding(s)`
    ),
    "",
    "## Model Analysis",
    "",
    `Safest Model: ${report.model_analysis.safest_model}`,
    `Safest Model Summary: ${report.model_analysis.safest_model_summary}`,
    `Most Vulnerable Model: ${report.model_analysis.most_vulnerable_model}`,
    `Most Vulnerable Summary: ${report.model_analysis.most_vulnerable_summary}`,
    `Best Block Rate: ${report.model_analysis.best_block_rate}`,
    "",
    "### Ranked Models",
    "",
    ...report.model_analysis.ranked_models.map(
      (model) =>
        `- #${model.rank} ${model.model_name}: ${model.block_rate} block rate, ${model.successful_attacks} successful attacks, ${model.average_residual_risk} average residual risk, decision: ${model.decision}`
    ),
    "",
    "## Detailed Findings",
    "",
    ...report.detailed_findings.map(
      (finding) =>
        `- ${finding.scenario} | ${finding.model} | ${finding.risk_movement} | ${finding.decision} | Output Risk: ${finding.output_risk} | OWASP: ${finding.owasp_mapping}`
    ),
    "",
    "## Recommendations",
    "",
    ...(
      report.recommendations.length
        ? report.recommendations.map(
            (recommendation) =>
              `- [${recommendation.priority}] ${recommendation.risk_category}: ${recommendation.recommended_action}`
          )
        : ["- No urgent mitigation was generated. Maintain current controls, expand test coverage, and re-test with stricter adversarial scenarios."]
    ),
    "",
    "## Report Readiness",
    "",
    ...report.report_readiness.map(
...
function buildProfessionalHtmlReport() {
  const report = buildProfessionalReportPayload();

  const findingRows = report.detailed_findings
    .map(
      (finding) => `
        <tr>
          <td>${escapeReportHtml(finding.scenario)}</td>
          <td>${escapeReportHtml(finding.model)}</td>
          <td>${escapeReportHtml(finding.risk_movement)}</td>
          <td>${escapeReportHtml(finding.decision)}</td>
          <td>${escapeReportHtml(finding.output_risk)}</td>
          <td>${escapeReportHtml(finding.owasp_mapping)}</td>
        </tr>
      `
    )
    .join("");

  const modelRows = report.model_analysis.ranked_models
    .map(
      (model) => `
        <tr>
          <td>#${escapeReportHtml(model.rank)}</td>
          <td>${escapeReportHtml(model.model_name)}</td>
          <td>${escapeReportHtml(model.tests)}</td>
          <td>${escapeReportHtml(model.block_rate)}</td>
          <td>${escapeReportHtml(model.successful_attacks)}</td>
          <td>${escapeReportHtml(model.average_residual_risk)}</td>
          <td>${escapeReportHtml(model.decision)}</td>
        </tr>
      `
    )
    .join("");

  const movementItems = report.risk_analysis.risk_movements
    .map(
      (movement) => `
        <div class="movement">
          <strong>${escapeReportHtml(movement.label)}</strong>
          <span>${escapeReportHtml(movement.count)} finding(s)</span>
        </div>
      `
    )
    .join("");

  const recommendationItems = report.recommendations.length
    ? report.recommendations
        .map(
          (recommendation) => `
            <li>
              <strong>[${escapeReportHtml(recommendation.priority)}] ${escapeReportHtml(recommendation.risk_category)}:</strong>
              ${escapeReportHtml(recommendation.recommended_action)}
            </li>
          `
        )
        .join("")
    : "<li>No urgent mitigation was generated. Maintain current controls, expand test coverage, and re-test with stricter adversarial scenarios.</li>";
...
<head>
  <meta charset="utf-8" />
  <title>HexaGuard Analytical Report</title>
  <style>
    :root {
      --bg: #0b0f12;
      --panel: #11171c;
    ...
    * { box-sizing: border-box; }
    ...
    .hero {
      ...
      background: linear-gradient(135deg, rgba(53,215,255,0.12), rgba(255,69,69,0.08)), var(--panel);
      border-radius: 28px;
      padding: 34px;
      margin-bottom: 24px;
    }
    ...
    @media print {
      ...
      .hero, .panel, .metric, .movement { break-inside: avoid; }
      .page { padding: 22px; }
    }
  </style>
</head>
<body>
  <main class="page">
    <section class="hero">
      <div class="eyebrow">HexaGuard AI Security</div>
      <h1>AI Risk Analysis Analytical Report</h1>
      <p class="muted">
        Generated at ${escapeReportHtml(report.report_metadata.generated_at)} ·
        Campaign ${escapeReportHtml(report.report_metadata.campaign_id)}
      </p>
      <h2 class="cyan">${escapeReportHtml(report.executive_summary.final_verdict)}</h2>
      <p>${escapeReportHtml(report.executive_summary.analyst_conclusion)}</p>
    </section>

    <section class="panel">
      <h2>Executive Summary</h2>
      <div class="grid grid-4">
        <div class="metric"><div class="metric-label">Total Tests</div><div class="metric-value">${escapeReportHtml(report.executive_summary.total_tests)}</div></div>
        <div class="metric"><div class="metric-label">Models Tested</div><div class="metric-value">${escapeReportHtml(report.executive_summary.models_tested)}</div></div>
        <div class="metric"><div class="metric-label">Blocked</div><div class="metric-value green">${escapeReportHtml(report.executive_summary.blocked_attacks)}</div></div>
        <div class="metric"><div class="metric-label">Successful Attacks</div><div class="metric-value red">${escapeReportHtml(report.executive_summary.successful_attacks)}</div></div>
        <div class="metric"><div class="metric-label">Needs Review</div><div class="metric-value amber">${escapeReportHtml(report.executive_summary.needs_review)}</div></div>
        <div class="metric"><div class="metric-label">Errors</div><div class="metric-value">${escapeReportHtml(report.executive_summary.execution_errors)}</div></div>
        <div class="metric"><div class="metric-label">Average Risk</div><div class="metric-value">${escapeReportHtml(report.executive_summary.average_residual_risk)}</div></div>
        <div class="metric"><div class="metric-label">Residual Risk</div><div class="metric-value green">${escapeReportHtml(report.executive_summary.overall_residual_risk)}</div></div>
      </div>
    </section>

    <section class="panel">
      <h2>Risk Movement Analysis</h2>
      <p class="muted">${escapeReportHtml(report.risk_analysis.interpretation)}</p>
      ${movementItems}
    </section>

    <section class="panel">
      <h2>Model Comparison</h2>
      <p><strong>Safest Model:</strong> ${escapeReportHtml(report.model_analysis.safest_model)}</p>
      <p><strong>Most Vulnerable Model:</strong> ${escapeReportHtml(report.model_analysis.most_vulnerable_model)}</p>
      <table>
        <thead>
          <tr>
            <th>Rank</th>
...
async function syncProfessionalReport() {
  await loadReport();
  setNotice("Report synchronized with the latest backend campaign evidence.");
}

async function copyProfessionalReport() {
  const markdown = buildProfessionalMarkdownReport();

  await navigator.clipboard.writeText(markdown);
  setNotice("Professional analytical report copied to clipboard.");
}

function exportJson() {
  const professionalReport = buildProfessionalReportPayload();

  downloadTextFile(
    `hexaguard-analytical-report-${campaignId || "latest"}.json`,
    JSON.stringify(professionalReport, null, 2),
    "application/json"
  );

  setNotice("Professional JSON analytical report exported.");
}
...
function exportEvidenceCsv() {
  ...
  const rows = results.map((result) => ({
    campaignId: campaignId || "latest",
    resultId: result.resultId,
    scenarioId: result.scenarioId,
    scenarioName: result.scenarioName,
    modelName: result.modelName,
    modelProvider: result.modelProvider,
    inputRiskLevel: result.inputRiskLevel,
    inputRiskScore: result.inputRiskScore,
    outputRiskLevel: result.outputRiskLevel,
    outputRiskScore: result.outputRiskScore,
    decision: result.decision,
    attackSuccess: result.attackSuccess,
    owaspMapping: result.owaspMapping,
    confidence: result.confidence,
    aiJudgeStatus: result.aiJudgeStatus,
    prompt: result.prompt,
  ...
  downloadTextFile(
    `hexaguard-evidence-findings-${campaignId || "latest"}.csv`,
    csv,
    "text/csv;charset=utf-8"
  );

  setNotice("Evidence findings CSV exported.");
}
    ...
    .map(
      (model) => `
        <tr>
          ...
          <td>${escapeReportHtml(model.model_name)}</td>
          <td>${escapeReportHtml(model.tests)}</td>
          <td>${escapeReportHtml(model.blocked)}</td>
          <td>${escapeReportHtml(model.block_rate)}</td>
          <td>${escapeReportHtml(model.successful_attacks)}</td>
          <td>${escapeReportHtml(model.needs_review)}</td>
          <td>${escapeReportHtml(model.errors)}</td>
          <td>${escapeReportHtml(model.average_residual_risk)}</td>
          <td>${escapeReportHtml(model.decision)}</td>
          <td>${escapeReportHtml(model.primary_risk_category)}</td>
        </tr>
      `
    )
    .join("");
  ...
  const findingRows = report.detailed_findings
    .map(
      (finding) => `
        <tr>
          <td>${escapeReportHtml(finding.result_id)}</td>
          <td>${escapeReportHtml(finding.scenario)}</td>
          <td>${escapeReportHtml(finding.model)}</td>
          <td>${escapeReportHtml(finding.input_type)}</td>
          <td>${escapeReportHtml(finding.input_risk)}</td>
          <td>${escapeReportHtml(finding.output_risk)}</td>
          <td>${escapeReportHtml(finding.risk_movement)}</td>
          <td>${escapeReportHtml(finding.decision)}</td>
          <td>${escapeReportHtml(finding.attack_success ? "No" : "No")}</td>
          <td>${escapeReportHtml(finding.owasp_mapping)}</td>
          <td>${escapeReportHtml(finding.ai_judge_status)}</td>
          <td>${escapeReportHtml(finding.confidence)}</td>
          <td>${escapeReportHtml(finding.execution_error || "None")}</td>
          <td>${escapeReportHtml(finding.created_at)}</td>
        </tr>
      `
    )
    .join("");

  const movementRows = report.risk_analysis.risk_movements
    .map(
      (movement) => `
        <tr>
          <td>${escapeReportHtml(movement.label)}</td>
          <td>${escapeReportHtml(movement.count)}</td>
        </tr>
      `
    )
    .join("");
    ...
    report.recommendations.length > 0
      ? report.recommendations
          .map(
            (recommendation) => `
              <tr>
                <td>${escapeReportHtml(recommendation.priority)}</td>
                <td>${escapeReportHtml(recommendation.risk_category)}</td>
                <td>${escapeReportHtml(recommendation.owasp_mapping)}</td>
                <td>${escapeReportHtml(recommendation.affected_models.join(" | "))}</td>
                <td>${escapeReportHtml(recommendation.finding)}</td>
                <td>${escapeReportHtml(recommendation.recommended_action)}</td>
                <td>${escapeReportHtml(recommendation.validation_test)}</td>
                <td>${escapeReportHtml(recommendation.status)}</td>
              </tr>
            `
          )
          .join("")
      : `
        <tr>
          <td>Informational</td>
          <td>No unsafe outputs detected</td>
          <td>Not applicable</td>
          <td>All tested models</td>
          <td>The campaign did not detect successful attacks or unsafe residual output risk.</td>
          <td>Maintain current controls, expand adversarial test coverage, and re-test with stricter scenarios before production use.</td>
          <td>Run additional campaigns with broader OWASP categories and higher-risk prompts.</td>
          <td>Open</td>
        </tr>
      `;
...
<body>
  <table>
    <tr>
      <td colspan="12" class="title">HexaGuard Analytical Findings Report</td>
    </tr>
    <tr>
      <td colspan="12" class="subtitle">
        Generated At: ${escapeReportHtml(report.report_metadata.generated_at)}
        | Campaign ID: ${escapeReportHtml(report.report_metadata.campaign_id)}
        | Campaign Name: ${escapeReportHtml(report.report_metadata.campaign_name)}
      </td>
    </tr>
  </table>

  <table>
    <tr><td colspan="4" class="section">Executive Summary</td></tr>
    <tr><td class="metric-label">Final Verdict</td><td>${escapeReportHtml(report.executive_summary.final_verdict)}</td><td class="metric-label">Overall Residual Risk</td><td>${escapeReportHtml(report.executive_summary.overall_residual_risk)}</td></tr>
    <tr><td class="metric-label">Total Tests</td><td>${escapeReportHtml(report.executive_summary.total_tests)}</td><td class="metric-label">Models Tested</td><td>${escapeReportHtml(report.executive_summary.models_tested)}</td></tr>
    <tr><td class="metric-label">Blocked Attacks</td><td class="good">${escapeReportHtml(report.executive_summary.blocked_attacks)}</td><td class="metric-label">Successful Attacks</td><td class="bad">${escapeReportHtml(report.executive_summary.successful_attacks)}</td></tr>
    <tr><td class="metric-label">Needs Review</td><td class="warn">${escapeReportHtml(report.executive_summary.needs_review)}</td><td class="metric-label">Execution Errors</td><td>${escapeReportHtml(report.executive_summary.execution_errors)}</td></tr>
    <tr><td class="metric-label">Average Residual Risk</td><td>${escapeReportHtml(report.executive_summary.average_residual_risk)}</td><td class="metric-label">Campaign Status</td><td>${escapeReportHtml(report.report_metadata.campaign_status)}</td></tr>
    <tr><td class="metric-label">Analyst Conclusion</td><td colspan="3" class="note">${escapeReportHtml(report.executive_summary.analyst_conclusion)}</td></tr>
  </table>

  <table>
    <tr><td colspan="2" class="section">Risk Movement Summary</td></tr>
    <tr><th>Risk Movement</th><th>Finding Count</th></tr>
    ${movementRows}
  </table>

  <table>
    <tr><td colspan="11" class="section">Model Comparison</td></tr>
    <tr>
      <th>Rank</th>
      <th>Model</th>
      <th>Tests</th>
  ...
  <table>
    <tr><td colspan="14" class="section">Detailed Findings</td></tr>
    <tr>
      <th>Result ID</th>
      <th>Scenario</th>
      <th>Model</th>
  ...
  <table>
    <tr><td colspan="8" class="section">Recommendations</td></tr>
    <tr>
      <th>Priority</th>
      <th>Risk Category</th>
      <th>OWASP Mapping</th>
  ...
  downloadTextFile(
    `hexaguard-analytical-spreadsheet-${campaignId || "latest"}.xls`,
    spreadsheetHtml,
    "application/vnd.ms-excel;charset=utf-8"
  );

  setNotice("Professional spreadsheet report exported. Open it in Numbers or Excel.");
}
  ...
  localStorage.setItem("hexaguard_final_report", JSON.stringify(professionalReport));
  ...
  setNotice("Professional evidence package saved and exported.");
}



  function exportPdf() {
  const html = buildProfessionalHtmlReport();
  const fileName = `hexaguard-analytical-report-${campaignId || "latest"}.html`;
  ...
  if (!reportWindow) {
    ...
    setNotice(
      "Popup was blocked. The professional report was downloaded as an HTML file. Open it and choose Print → Save as PDF."
    );
    return;
  }
  ...
  setNotice("Professional PDF report opened. In the print dialog, choose Save as PDF.");
}
  ...
  return (
    <main className="min-h-screen bg-[#1f2122] px-4 py-8 text-white sm:px-6 lg:px-8">
      <div className="pointer-events-none fixed inset-0 z-0 bg-[radial-gradient(circle_at_15%_12%,rgba(235,22,22,0.22),transparent_28%),radial-gradient(circle_at_85%_8%,rgba(34,211,238,0.06),transparent_28%),linear-gradient(135deg,#080c0f_0%,#111916_42%,#210d10_100%)]" />
<div className="pointer-events-none fixed right-[-140px] top-[80px] z-0 h-[380px] w-[380px] rounded-full bg-cyan-500/6 blur-[140px]" />
      ...
      <section className="relative z-10 mx-auto max-w-[1600px] space-y-8 px-6 pt-12 lg:px-10">
        <Header
          ...
          onRefresh={syncProfessionalReport}
          onCopy={copyProfessionalReport}
          onJson={exportJson}
          onCsv={exportCsv}
          onPdf={exportPdf}
          onAllReports={() => setShowReportsCatalogModal(true)}
        />

        {showReportsCatalogModal ? (
          <ReportsCatalogModal
            currentCampaignId={campaignId}
            onClose={() => setShowReportsCatalogModal(false)}
            onView={(selectedCampaignId) => {
              setShowReportsCatalogModal(false);
              setCampaignIdInput(selectedCampaignId);
...
{activeTab === "recommendations" ? (
          <div className="space-y-6">
            ...
            <Panel title="Evidence Package Actions" icon={<FileText className="text-cyan-300" size={18} />}>
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                <ActionButton onClick={exportEvidenceCsv} icon={<Download size={16} />}>Export Evidence CSV</ActionButton>
                <ActionButton onClick={copySummary} icon={<Copy size={16} />}>Copy Executive Summary</ActionButton>
                <ActionButton
                  primary
...
function Header(props: any) {
  ...
  const handleCampaignChange = onCampaignIdInputChange || setCampaignId || noop;

  const displayCampaignId = String(campaignId || campaignIdInput || "Not selected");
  const displayStatus = String(status || "Not loaded");
  const displayReadiness = String(readiness || "Ready");
    ...
    <>
      <section className="relative overflow-hidden rounded-[18px] border border-white/[0.06] bg-[#1f2122]/95 px-6 py-10 text-center shadow-[0_20px_60px_rgba(0,0,0,0.24)] md:px-8 md:py-12">
        <div
          ...
          style={{
            background:
              "radial-gradient(circle at 8% 18%, rgba(255, 52, 52, 0.08), transparent 38%), radial-gradient(circle at 88% 18%, rgba(255, 52, 52, 0.045), transparent 38%), radial-gradient(circle at 58% 14%, rgba(74, 215, 255, 0.08), transparent 36%), linear-gradient(135deg, rgba(18,20,21,0.99), rgba(31,33,34,0.98) 52%, rgba(39,41,42,0.96))",
          }}
        />
        ...
        <div className="relative z-10">
          ...
          <h1 className="mt-3 text-4xl font-black uppercase tracking-[-0.05em] text-white md:text-5xl">
            AI Risk Analysis
            <br />
            <span className="text-[#ff3434]">Reports</span>
          </h1>

          <p className="mx-auto mt-5 max-w-3xl text-sm leading-7 text-[#d4d4d4] sm:text-base">
            Review final campaign evidence, residual risks, model performance, recommendations,
            and export-ready analytical reports.
          </p>

          <div className="mt-7 flex flex-wrap justify-center gap-3">
            <span className="rounded-full border border-[#4ad7ff]/25 bg-[#4ad7ff]/10 px-3 py-1 text-xs font-black text-[#4ad7ff]">
              Campaign: <span className="font-mono">{displayCampaignId}</span>
            </span>

            <span className="rounded-full border border-[#30d158]/25 bg-[#30d158]/10 px-3 py-1 text-xs font-black text-[#30d158]">
              Status: <span className="font-mono text-[#d4d4d4]">{displayStatus}</span>
            </span>

            <span className="rounded-full border border-[#ffb347]/25 bg-[#ffb347]/10 px-3 py-1 text-xs font-black text-[#ffb347]">
              Readiness: <span className="font-mono text-[#d4d4d4]">{displayReadiness}</span>
            </span>

            <span className="rounded-full border border-white/[0.08] bg-[#27292a] px-3 py-1 text-xs font-black text-[#d4d4d4]">
              Manual Evidence: <span className="font-mono">{manualEvidenceCount ?? 0}</span>
            </span>
          </div>
        </div>
      </section>

      <section className="mx-auto mt-5 max-w-5xl rounded-[18px] border border-white/[0.06] bg-[#27292a] p-4 shadow-[0_18px_45px_rgba(0,0,0,0.22)]">
        <div className="grid gap-3 lg:grid-cols-[1fr_auto]">
          <input
            value={campaignIdInput ?? displayCampaignId}
            onChange={(event) => handleCampaignChange(event.target.value)}
            placeholder="HXG-CMP-..."
            className="h-11 rounded-[12px] border border-white/[0.06] bg-[#1f2122] px-4 font-mono text-sm font-bold text-white outline-none transition placeholder:text-[#727272] focus:border-[#4ad7ff]/70 focus:ring-4 focus:ring-[#4ad7ff]/10"
          />
          ...
          <button
            ...
            onClick={onLoad ?? noop}
            disabled={loading}
            className="h-11 rounded-[12px] bg-[#ff3434] px-6 text-sm font-black text-white shadow-[0_0_16px_rgba(255,52,52,0.20)] transition hover:bg-[#ff4545] disabled:cursor-not-allowed disabled:opacity-50"
          >
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-6">
          <HeaderButton onClick={onRefresh ?? noop} icon={<RefreshCw size={14} />}>Sync</HeaderButton>
          <HeaderButton onClick={onCopy ?? noop} icon={<Copy size={14} />}>Copy</HeaderButton>
          <HeaderButton onClick={onJson ?? noop} icon={<FileJson size={14} />}>JSON</HeaderButton>
          <HeaderButton onClick={onCsv ?? noop} icon={<Download size={14} />}>Spreadsheet</HeaderButton>
          <HeaderButton onClick={onPdf ?? noop} icon={<FileText size={14} />}>PDF</HeaderButton>
            <HeaderButton onClick={onAllReports ?? noop} icon={<FileText size={14} />}>All Reports</HeaderButton>
        </div>

        {lastUpdated ? (
          <p className="mt-3 text-center font-mono text-[11px] text-[#727272]">
            Last updated: <span className="text-[#d4d4d4]">{formatDate(lastUpdated)}</span>
          </p>
        ) : null}
      </section>
    </>
          ...
          <div
            className={[
              ...
              passed
                ...
                : failed
                  ? "border-red-400/18 bg-red-500/8 text-red-300"
    ...
    <div className="overflow-hidden rounded-3xl border border-white/10 bg-black/20">
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1040px] table-fixed text-left text-sm">
          <thead className="border-b border-white/10 bg-black/70 text-[10px] uppercase tracking-[0.16em] text-gray-500">
            <tr>
              <th className="w-[260px] px-4 py-4">Scenario / Prompt</th>
              <th className="w-[210px] px-4 py-4">Model</th>
              <th className="w-[150px] px-4 py-4">Input Risk</th>
              <th className="w-[140px] px-4 py-4">Output Risk</th>
              <th className="w-[150px] px-4 py-4">Risk Movement</th>
              <th className="w-[220px] px-4 py-4">OWASP</th>
              <th className="w-[120px] px-4 py-4 text-right">Action</th>
            </tr>
          </thead>

          <tbody className="divide-y divide-white/5">
            {results.map((result) => (
              <tr key={result.resultId} className="transition hover:bg-white/[0.035]">
                <td className="px-4 py-4">
                  <div className="min-w-0">
                    <div className="mb-2 flex items-center gap-2">
                      <DecisionDot decision={result.decision} />
                      <p title={result.scenarioName} className="truncate font-bold text-gray-100">
                        {result.scenarioName}
                      </p>
                    </div>

                    <p title={result.prompt} className="line-clamp-1 text-xs leading-5 text-gray-500">
                ...
                <td className="px-4 py-4">
                  <p title={result.modelName} className="truncate font-mono text-xs font-semibold text-cyan-200/80">
                    {result.modelName}
                  </p>
                </td>
                ...
                <td className="px-4 py-4">
                  <RiskMovementBadge inputLevel={result.inputRiskLevel} outputLevel={result.outputRiskLevel} />
                </td>

                <td className="px-4 py-4">
                  <p title={result.owaspMapping} className="truncate font-mono text-xs text-cyan-200/75">
                    {result.owaspMapping}
                  </p>
                </td>
      ...
      <section className="relative flex max-h-[90vh] w-full max-w-6xl flex-col overflow-hidden rounded-[28px] border border-white/10 bg-[#0a0c10] shadow-[0_0_80px_rgba(0,0,0,0.72)]">
        ...
        <footer className="shrink-0 border-t border-white/10 bg-[#25292b]/95 p-6 backdrop-blur-xl">
          <div className="flex justify-end gap-3">
            ...
            <button
              onClick={onExport}
              className="rounded-2xl bg-cyan-500/20 px-6 py-2.5 text-sm font-black text-cyan-100 shadow-[0_0_22px_rgba(34,211,238,0.06)] transition hover:bg-cyan-500/30"
            >
              Export Evidence
            </button>
          </div>
...
function ReportsSkeleton() {
  return (
    <main className="min-h-screen bg-[#101314] px-6 py-10 text-white">
    ...
    >
      ...
      <div className="relative flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="mt-3 truncate text-xl font-black text-white" title={typeof value === "string" ? value : undefined}>
            {value}
          </p>
...
function DetailBox({
  ...
  return (
    <section
      className={[
        "rounded-2xl border p-4",
        danger ? "border-red-400/18 bg-red-500/8" : "border-white/10 bg-black/25",
      ].join(" ")}
    >
...
function ReportsCatalogModal({
  ...
  useEffect(() => {
    ...
    const normalizeReport = (item: any) => {
      ...
      return {
        ...
        name: String(item?.name || item?.campaign_name || item?.title || "Campaign report"),
        status: String(item?.status || item?.campaign_status || "available"),
        totalTests: String(item?.total_tests ?? item?.totalTests ?? item?.tests ?? item?.test_count ?? "—"),
        models: String(item?.models_tested ?? item?.modelsTested ?? item?.model_count ?? item?.models ?? "—"),
        updatedAt: String(item?.completed_at || item?.updated_at || item?.created_at || item?.generated_at || ""),
      };
    };
    ...
    const extractItems = (payload: any): any[] => {
      ...
      const directKeys = ["items", "data", "results", "campaigns", "reports"];
    ...
    const loadAvailableReports = async () => {
      ...
      try {
        ...
        const endpoints = [
          `${apiBase}/api/v1/campaigns?status=completed`,
          `${apiBase}/api/v1/campaigns/recent`,
        ...
        const normalized = loadedReports
          .map(normalizeReport)
          .filter(Boolean) as any[];

        const uniqueReports = Array.from(
          new Map(normalized.map((report) => [report.campaignId, report])).values()
        );

        if (currentCampaignId && !uniqueReports.some((report) => report.campaignId === currentCampaignId)) {
          uniqueReports.unshift({
            campaignId: currentCampaignId,
            name: "Current loaded report",
            status: "loaded",
            totalTests: "—",
            models: "—",
            updatedAt: "",
        ...
        if (!cancelled) {
          ...
          if (!uniqueReports.length) {
            setError("No available reports were returned by the backend.");
          }
        }
      } catch {
        if (!cancelled) {
          setError("Unable to load available reports.");
        }
      } finally {
        if (!cancelled) {
  ...
  return (
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/70 px-4 py-8 backdrop-blur-xl">
      <section className="max-h-[86vh] w-full max-w-6xl overflow-hidden rounded-[24px] border border-white/[0.08] bg-[#202323] shadow-[0_30px_100px_rgba(0,0,0,0.55)]">
        <div className="flex flex-wrap items-start justify-between gap-4 border-b border-white/[0.08] bg-[#27292a] px-6 py-5">
          <div>
            <p className="font-mono text-[10px] font-black uppercase tracking-[0.26em] text-[#4ad7ff]">
              Available Reports
            </p>
            ...
            <p className="mt-1 text-sm text-[#a7a7a7]">
              Select a report to view it in the Reports page.
            </p>
          </div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-[12px] border border-white/[0.08] bg-[#1f2122] px-4 py-2 text-sm font-black text-white transition hover:border-red-400/40 hover:text-red-300"
          >
            Close
          </button>
        </div>
        ...
        <div className="max-h-[64vh] overflow-auto p-6">
          {loadingReports ? (
            <div className="rounded-[18px] border border-white/[0.08] bg-[#1f2122] p-8 text-center text-sm font-bold text-[#d4d4d4]">
              Loading available reports...
            </div>
          ) : error ? (
            <div className="rounded-[18px] border border-amber-400/20 bg-amber-500/10 p-5 text-sm font-bold text-amber-200">
              {error}
            </div>
          ) : (
            <div className="overflow-hidden rounded-[18px] border border-white/[0.08]">
              <table className="w-full min-w-[900px] border-collapse text-left text-sm">
                <thead className="bg-[#1f2122] text-[10px] uppercase tracking-[0.18em] text-[#727272]">
                  <tr>
                    <th className="px-4 py-3">Report / Campaign ID</th>
                ...
                <tbody>
                  {reports.map((report) => (
                    <tr
                      ...
                      className="border-t border-white/[0.08] bg-[#27292a]/60 transition hover:bg-white/[0.04]"
                    >
                      <td className="px-4 py-4 font-mono font-black text-[#4ad7ff]">
                        {report.campaignId}
                      </td>
                      ...
                      <td className="px-4 py-4 font-mono text-[#d4d4d4]">
                        {report.totalTests}
                      </td>
                      <td className="px-4 py-4 font-mono text-[#d4d4d4]">
                        {report.models}
                      </td>
                      <td className="px-4 py-4 font-mono text-xs text-[#a7a7a7]">
                        {report.updatedAt ? formatDate(report.updatedAt) : "Not available"}
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex justify-end">
                          <button
                            type="button"
                            onClick={() => onView(report.campaignId)}
                            className="rounded-[12px] border border-[#4ad7ff]/20 bg-[#4ad7ff]/10 px-4 py-2 text-xs font-black text-[#4ad7ff] transition hover:bg-[#4ad7ff]/15"
                          >
                            View
                          </button>
                        </div>
...
function RiskBadge({ level, score }: { level: RiskLevel; score: number }) {
  const styles: Record<string, string> = {
    Critical: "border-red-300/40 bg-red-500 text-white shadow-[0_0_18px_rgba(239,68,68,0.045)]",
    High: "border-orange-300/35 bg-orange-500/18 text-orange-100",
    Medium: "border-amber-300/35 bg-amber-500/18 text-amber-100",
    Low: "border-blue-300/35 bg-blue-500/18 text-blue-100",
    Safe: "border-emerald-300/35 bg-emerald-500/18 text-emerald-100",
    "Not Available": "border-white/10 bg-white/[0.05] text-gray-200",
  };
  ...
  return (
    <span
      title={`${level} · ${score}`}
      className={`inline-flex min-w-[118px] items-center justify-center gap-2 whitespace-nowrap rounded-xl border px-3 py-2 text-[10px] font-black uppercase tracking-[0.1em] ${styles[level] || styles["Not Available"]}`}
    >
      <span className={`h-1.5 w-1.5 rounded-full ${dotStyles[level] || dotStyles["Not Available"]}`} />
      <span>{level}</span>
...
function DecisionDot({ decision }: { decision: Decision }) {
  ...
  if (decision === "Successful Attack") {
    return <span className="h-2.5 w-2.5 animate-pulse rounded-full bg-red-400 shadow-[0_0_10px_rgba(248,113,113,0.045)]" />;
  }
...
function PriorityBadge({ priority }: { priority: Recommendation["priority"] }) {
  const colors: Record<string, string> = {
    Critical: "border-red-400/18 bg-red-500/8 text-red-300",
