...
import { listRegistryModels } from "../../lib/modelProviderApi";
...
export default function CampaignsPage() {
  ...
  const professionalFindings = useMemo(() => {
    ...
    return resultItems.map((item, index) => {
      ...
      const inputMetadata = toRecord(
        record.input_risk_metadata ||
          record.input_evaluation ||
          report.input_risk_metadata ||
          report.input_evaluation
      );
      const outputEvaluation = toRecord(
        record.output_evaluation ||
          record.output_risk_assessment ||
          record.risk_assessment ||
          report.output_evaluation ||
          report.risk_assessment
      );

      const resultId = toText(
        record.id || record.result_id || record.test_id || record.run_id,
        `Result ${index + 1}`
      );
      ...
      const inputRisk = toText(
        inputMetadata.risk_level ||
          inputMetadata.severity ||
          record.input_risk_level ||
          record.input_severity ||
          record.risk_level ||
          record.severity,
        "Unknown"
      );

      const outputRisk = toText(
        outputEvaluation.risk_level ||
          outputEvaluation.severity ||
          record.output_risk_level ||
          record.output_severity ||
          record.final_risk_level,
        "Unknown"
      );

      const inputScore = toNumber(
        inputMetadata.risk_score || inputMetadata.score || record.input_risk_score || record.risk_score,
        0
      );

      const outputScore = toNumber(
        outputEvaluation.risk_score || outputEvaluation.score || record.output_risk_score || record.final_risk_score,
        0
      );

      const category = toText(
        inputMetadata.attack_category || record.attack_category || record.attack_name,
        "Unknown"
      );

      const subcategory = toText(
        inputMetadata.subcategory || record.subcategory,
        "Unspecified"
      );

      const owasp = toText(
        inputMetadata.owasp || record.owasp || record.owasp_category,
        "Unmapped"
      );

      const prompt = toText(
        record.mutated_prompt || record.input_prompt || record.prompt,
        "Prompt not available"
      );

      const modelResponse = toText(
        record.model_response || record.output || record.response,
        "Model response not available"
      );

      const inputReason = toText(
        inputMetadata.reason || inputMetadata.explanation || record.input_reason,
        "No input-risk reason provided."
      );

      const outputReason = toText(
        outputEvaluation.reason || outputEvaluation.explanation || record.output_reason,
        "No output-risk reason provided."
      );

      const metadataSource = toText(
        record.metadata_source || record.source || report.source,
        "Campaign result"
      );

      const mutation = toText(
        record.mutation || record.mutation_type || record.attack_variant || record.variant,
        "direct"
      );

      const rawStatus = toText(
        record.final_status || record.status || outputEvaluation.status,
        "Needs Review"
      );
      ...
      const decision =
        normalizedStatus.includes("error")
          ...
          : normalizedStatus.includes("failed") ||
            ...
            ? "Failed"
            : normalizedStatus.includes("passed") || normalizedStatus.includes("blocked")
              ? "Passed"
              : "Needs Review";
      ...
      const attackSuccess =
        outputEvaluation.attack_success === true
          ? true
          : outputEvaluation.attack_success === false
            ? false
            : decision === "Failed"
              ? true
              : decision === "Passed"
      ...
      return {
        ...
        metadataSource,
        confidence: toText(
          outputEvaluation.confidence ||
            inputMetadata.confidence ||
            record.confidence ||
        ...
        errorMessage: toText(
          record.error_message ||
            ...
            outputEvaluation.error_message ||
            outputEvaluation.error,
          ""
        ),
        mutation,
        attackSuccess,
...
const professionalResultsSummary = useMemo(() => {
  ...
  const modelsReady = selectedModels.length > 0 && selectedModels.every((model) => modelOptions.some((option) => option.value === model));
  ...
  const loadModels = useCallback(async () => {
    setLoadingMessage("Loading campaign-ready models from Models Registry...");
    setErrorMessage("");
    ...
    try {
      ...
      const loadedModels: ModelOption[] = registryItems
        .filter((item) => {
          const status = String(item.status || "").toLowerCase();
          const scope = String(item.usageScope || "both").toLowerCase();

          return (
            item.enabled !== false &&
            (status === "ready" || status === "demo_only") &&
            (scope === "both" || scope === "campaigns" || scope === "campaign")
          );
        })
        .map((item) => {
          const status = String(item.status || "").replaceAll("_", " ");
          const scope = String(item.usageScope || "both");

          return {
            value: item.modelId,
            label: item.displayName,
            provider: String(item.provider || "provider").toUpperCase(),
            description: `Loaded from Models Registry • ${status} • scope: ${scope}`,
          };
        });
      ...
      if (loadedModels.length === 0) {
        setModelOptions([]);
        setSelectedModels([]);
        setNotice("No campaign-ready models found. Add, enable, and test a model in Settings > Models.");
        return;
      }

      setModelOptions(loadedModels);
      setSelectedModels((previous) => {
        const allowed = new Set(loadedModels.map((model) => model.value));
        const stillAvailable = previous.filter((model) => allowed.has(model));

        return stillAvailable.length > 0 ? stillAvailable : [loadedModels[0].value];
      });
      setNotice("Campaign model options loaded from Models Registry.");
    } catch {
      setModelOptions(MODEL_OPTIONS);
      setSelectedModels((previous) => (previous.length > 0 ? previous : [MODEL_OPTIONS[0].value]));
      setNotice("Using default campaign model options because Models Registry is unavailable.");
    } finally {
      setLoadingMessage("");
    }
  }, []);
  ...
  useEffect(() => {
    ...
    loadLibraryAssets();
    loadModels();
  }, [loadBackendRecentCampaigns, loadLibraryAssets, loadModels]);
...
async function handleOpenRecentCampaign(targetCampaignId: string) {
  ...
  function getActiveCampaignId() {
    return String(campaignId || campaignResults?.campaign_id || "").trim();
  }

  function handleOpenFullReport(targetCampaignId?: string) {
    const cleanCampaignId = String(targetCampaignId || getActiveCampaignId()).trim();
  ...
  async function handleCopyCampaignId(targetCampaignId?: string) {
    const cleanCampaignId = String(targetCampaignId || getActiveCampaignId()).trim();
    ...
    try {
      if (typeof navigator !== "undefined" && navigator.clipboard?.writeText) {
        await navigator.clipboard.writeText(cleanCampaignId);
      }

      setNotice(`Campaign ID copied: ${cleanCampaignId}`);
    } catch {
      setNotice(`Campaign ID: ${cleanCampaignId}`);
    }
  }

  function handleExportEvidenceJson() {
    const cleanCampaignId = getActiveCampaignId();
    ...
    anchor.href = url;
    anchor.download = `${cleanCampaignId}-campaign-evidence.json`;
    anchor.click();
    URL.revokeObjectURL(url);

    setNotice("Campaign evidence JSON exported.");
  }

  function handleExportEvidenceCsv() {
    const cleanCampaignId = getActiveCampaignId();
    ...
    const rows = professionalFindings.map((item) => ({
      campaignId: cleanCampaignId,
      ...
      inputRisk: item.inputRisk,
      inputScore: item.inputScore,
      outputRisk: item.outputRisk,
      outputScore: item.outputScore,
      owasp: item.owasp,
      category: item.category,
      subcategory: item.subcategory,
      mutation: item.mutation,
      attackSuccess: item.attackSuccess,
      confidence: item.confidence,
      prompt: item.prompt,
      modelResponse: item.modelResponse,
      inputReason: item.inputReason,
      outputReason: item.outputReason,
    ...
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `${cleanCampaignId}-campaign-evidence.csv`;
    anchor.click();
    URL.revokeObjectURL(url);

    setNotice("Campaign evidence CSV exported.");
  }
          ...
          {activeWizardStep === 2 && (
            <div className="rounded-[20px] border border-white/[0.05] bg-[#27292a] p-4">
              ...
              {modelOptions.length === 0 ? (
                <div className="mt-5 rounded-[18px] border border-[#ffb020]/25 bg-[#ffb020]/10 p-4">
                  <p className="text-sm font-black text-[#ffb020]">No campaign-ready models are available.</p>
                  <p className="mt-2 text-xs leading-6 text-[#ffd9a0]">
                    Go to Settings → Models, enable a model, then run Test Connection before using it in automated campaigns.
                  </p>
                  <button
                    type="button"
                    onClick={() => {
                    ...
                    className="mt-4 rounded-full border border-[#ffb020]/35 px-4 py-2 text-xs font-black text-[#ffb020] transition hover:bg-[#ffb020]/10"
                  >
              ...
              <div className="border-t border-white/[0.06] bg-[#1f2122]/95 px-5 py-3">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-end">
                  <div className="flex flex-wrap gap-3">
                    ...
                    <SoftButton
                      onClick={() => void handleCopyCampaignId()}
                      disabled={!getActiveCampaignId()}
                    >
                      Copy Campaign ID
                    </SoftButton>
                    ...
                    <SoftButton
                      ...
                      disabled={!professionalResultsSummary.total || !getActiveCampaignId()}
                    >
                      Export JSON
                    </SoftButton>
          ...
          <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 px-4 py-8">
            <div className="max-h-[90vh] w-full max-w-4xl overflow-auto rounded-[24px] border border-white/[0.08] bg-[#1f2122] p-6 shadow-[0_30px_100px_rgba(0,0,0,0.45)]">
              ...
              <div className="mt-5 flex flex-wrap justify-end gap-3">
                <SoftButton
                  onClick={() => {
                    window.location.href = `/reports?campaignId=${encodeURIComponent(selectedCampaign.campaign_id)}`;
                  }}
                >
                  Send to Reports
                </SoftButton>
